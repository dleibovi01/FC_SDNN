...
class Solver{
    ...
    */
    ...
    void solve_sdnn(double dt, double CFL, bool adaptive, bool visc)
    {
        ...
        while (t < T)
        {
            ...
            if(visc)
            {
                ...
                updateVisc(&mesh, svw_m, pde, sp_diffs, phys_unknowns, stages, 
                    0); 
            }
    ...
    void solve_sdnn_fast(double dt, double CFL, bool adaptive, bool visc)
    {
        std::cout << "Solver" << std::endl;
        double t = 0.0;
        double T = pde.getT();
        auto patches = mesh.getPatches();
        const int npatches = sp_diffs.size();
        int phys_unknowns = pde.getPhysUnknowns();
        int stages = ts.getStages();
        pde.getIC()(&mesh);
        ...
        const int C = 27;
        const int N = 101;
        // std::vector<std::array<std::complex<double>, N + C> > cons_fft;
        // std::array<std::complex<double>, N + C> arsray0;
        // std::array<std::complex<double>, N + C> array1;
        // std::array<std::complex<double>, N + C> array2;
        // cons_fft.push_back(array0);
        // cons_fft.push_back(array1);
        // cons_fft.push_back(array2);
        // std::array<std::vector<std::complex<double> * >, npatches >cons_fft;
        // std::array<std::vector<std::complex<double> * >, npatches > flux_fft;
        std::vector<std::complex<double> * > cons_fft[npatches] ;
        std::vector<std::complex<double> * > flux_fft[npatches] ;
        for(int i = 0; i < npatches; i++)
        {
            // cons_fft.push_back(new std::vector<std::complex<double> *>);
            // flux_fft.push_back(new std::vector<std::complex<double> *>);
            for(int j = 0; j < phys_unknowns*stages; j++)
            {
                
                cons_fft[i].push_back(new std::complex<double>[N+C]);
            }
            for(int j = 0; j < 2*phys_unknowns; j++)
            {
                flux_fft[i].push_back(new std::complex<double>[N+C]);
            }
        }
